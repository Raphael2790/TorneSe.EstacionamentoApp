...
public class UsuarioValidator : AbstractValidator<Usuario>
{
    public UsuarioValidator()
    {
        RuleFor(x => x.Nome)
            ...
            .MaximumLength(100)
            .WithMessage("O nome do usuário não pode ter mais que 100 caracteres.");

        RuleFor(x => x.Login)
            .NotEmpty()
            .WithMessage("O login do usuário não pode ser vazio.")
            .MaximumLength(100)
            .WithMessage("O login do usuário não pode ter mais que 100 caracteres.");

        RuleFor(x => x.Email)
            .NotEmpty()
            .WithMessage("O e-mail do usuário não pode ser vazio.")
            .MaximumLength(100)
            .WithMessage("O e-mail do usuário não pode ter mais que 100 caracteres.");

        RuleFor(x => x.Senha)
            .NotEmpty()
            .WithMessage("A senha do usuário não pode ser vazia.")
            .MaximumLength(100)
            .WithMessage("A senha do usuário não pode ter mais que 100 caracteres.");
